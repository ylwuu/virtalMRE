function [bvals,bvecs,Manufacturer,nii_file_name,varargout] = dicm2nii(src, niiFolder, fmt)
...
if isfield(h.(fnames{1}),'bval')
    for ind_bvals = 1:length(fnames)
        bvals{ind_bvals} = h.(fnames{ind_bvals}).bval;
    end
else
    bvals = nan;
end
if isfield(h.(fnames{1}),'bvec')
    for ind_bvecs = 1:length(fnames)
        bvecs{ind_bvecs} = h.(fnames{ind_bvecs}).bvec;
    end
else 
    bvecs = nan;
end
Manufacturer = h.(fnames{1}).Manufacturer;
nii_file_name = fnames;
